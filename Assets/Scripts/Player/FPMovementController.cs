...
public class FPMovementController : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {  
            if (!IsRunning)
            {
                rb.velocity = jumpDirection * walkSpeed * Time.fixedDeltaTime * 100 + YAxisGravity; 
            }
            else
            {
                 rb.velocity = jumpDirection * runSpeed * Time.fixedDeltaTime * 100 + YAxisGravity; 
            }
        }
        else 
        {
    ...
    {
        if (IsGrounded() && rb.velocity.magnitude > 1.0f && !audioSource.isPlaying)
        {
            if (IsRunning)
            {
